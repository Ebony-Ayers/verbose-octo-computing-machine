...
void threadFunction(unsigned char* imageData, const float& threshold, const size_t& width, const size_t& height, const float& viewWindowTopLeftCornerX, const float& viewWindowTopLeftCornerY, const float& viewWindowBottomRightCornerX, const float& viewWindowBottomRightCornerY, const int& subdivisionResolution, const float& subdivisionThreshold)
{
	...
	//const float cumulativeXIncrement = viewWindowBottomRightCornerX - viewWindowTopLeftCornerX; //unused variable
	...
	const size_t subdivisionEndIndex = subdivisionResolutionSizeT * subdivisionResolutionSizeT;
	...
	{
		//single loop for better branch prediction
		for(size_t i = startIndex; i < endIndex; i++)
		{
			...
			//branchless incrementing of x and y
			x += xIncrement;
	...
	else
	{
		...
		for(size_t i = startIndex; i < endIndex; i++)
		{
			...
			else if(res < subdivisionThreshold)
			{
				...
				float newX = x;
				float newY = y;
				for(size_t j = 0; j < subdivisionEndIndex; j++)
				{
					...
					if(j % static_cast<size_t>(subdivisionResolutionSizeT) == 0)
					{
						newX = x;
					}
					newY -= smallYIncrement * (j % static_cast<size_t>(subdivisionResolutionSizeT) == 0);
				}
			}
			...
			x += xIncrement;
			//x -= cumulativeXIncrement * (i % static_cast<size_t>(width) == 0);
			//this will compile down to cmov which is branchless
			if(i % static_cast<size_t>(width) == 0)
			{
				x = startX;
			}
